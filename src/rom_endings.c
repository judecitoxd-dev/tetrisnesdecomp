...
#define CONCERT_FIRST 0x16u
#define CONCERT_LAST 0x2au
#define CONCERT_COUNT (CONCERT_LAST - CONCERT_FIRST + 1u)
#define MAX_OAM_ENTRIES 64

typedef struct EndingSprite {
    SDL_Texture *texture;
    int width;
    int height;
    int min_x;
...
static EndingSprite g_concert[CONCERT_COUNT];
...
static EndingSprite build_sprite(SDL_Renderer *renderer, const NesRom *rom,
                                 size_t data_offset,
...
void tetris_rom_endings_free(void) {
    for (unsigned index = 0; index < CONCERT_COUNT; ++index) {
        if (g_concert[index].texture)
            SDL_DestroyTexture(g_concert[index].texture);
        memset(&g_concert[index], 0, sizeof(g_concert[index]));
    }
}

bool tetris_rom_endings_load(SDL_Renderer *renderer, const NesRom *rom) {
    EndingSprite loaded[CONCERT_COUNT];
    uint8_t palette[32];
    bool any = false;
    memset(loaded, 0, sizeof(loaded));
    if (!renderer || !rom || !rom->exact_supported_dump ||
        !load_ending_palette(rom, palette)) {
        tetris_rom_endings_free();
        return false;
    }
    for (unsigned index = CONCERT_FIRST; index <= CONCERT_LAST; ++index) {
        size_t data_offset;
        if (lookup_data_offset(rom, index, &data_offset)) {
            loaded[index - CONCERT_FIRST] =
                build_sprite(renderer, rom, data_offset, palette);
            if (loaded[index - CONCERT_FIRST].texture) any = true;
        }
    }
    tetris_rom_endings_free();
    memcpy(g_concert, loaded, sizeof(g_concert));
    return any;
}

bool tetris_rom_concert_sprite_available(unsigned sprite_index) {
    return sprite_index >= CONCERT_FIRST && sprite_index <= CONCERT_LAST &&
           g_concert[sprite_index - CONCERT_FIRST].texture != NULL;
}
...
void tetris_rom_concert_sprite_render(SDL_Renderer *renderer,
    ...
    const EndingSprite *sprite;
    if (!renderer || !tetris_rom_concert_sprite_available(sprite_index)) return;
    sprite = &g_concert[sprite_index - CONCERT_FIRST];
