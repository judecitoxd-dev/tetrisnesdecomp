...
static int test_nested_jsr_uses_stack_depth(void) {
    Memory memory;
    memset(&memory, 0, sizeof(memory));
    memory.bytes[0x8000] = 0x20; /* JSR $9000: 6 cycles */
    memory.bytes[0x8001] = 0x00;
    memory.bytes[0x8002] = 0x90;
    memory.bytes[0x8003] = 0x60; /* top-level RTS: 6 cycles */
    memory.bytes[0x9000] = 0xE6; /* INC $20: 5 cycles */
    memory.bytes[0x9001] = 0x20;
    memory.bytes[0x9002] = 0x60; /* nested RTS: 6 cycles */
    return run_call(0x8000u, &memory, 23u, 1u);
}

int main(void) {
    if (test_every_address_boundary() != 0) return 1;
    if (test_nested_jsr_uses_stack_depth() != 0) return 1;
    puts("CPU call-frame regression tests passed.");
    return 0;
}
