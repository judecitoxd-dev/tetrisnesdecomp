...
#define MAX_OAM_ENTRIES 256

typedef struct TypeASprite {
    SDL_Texture *texture;
    int width;
    int height;
    int min_x;
...
static const unsigned TYPE_A_SPRITE_INDICES[] = {
    0x23u, 0x24u,
    0x3eu, 0x3fu, 0x40u,
    0x41u, 0x42u, 0x43u,
    0x44u, 0x45u, 0x46u,
...
#define TYPE_A_SPRITE_COUNT \
    (sizeof(TYPE_A_SPRITE_INDICES) / sizeof(TYPE_A_SPRITE_INDICES[0]))

static TypeASprite g_type_a[TYPE_A_SPRITE_COUNT];
...
static int slot_for(unsigned sprite_index) {
    for (size_t slot = 0; slot < TYPE_A_SPRITE_COUNT; ++slot) {
        if (TYPE_A_SPRITE_INDICES[slot] == sprite_index) return (int)slot;
    }
    return -1;
}
...
static TypeASprite build_sprite(SDL_Renderer *renderer, const NesRom *rom,
                                size_t data_offset,
...
void tetris_rom_type_a_sprite_render(SDL_Renderer *renderer,
    ...
    const TypeASprite *sprite;
    const int slot = slot_for(sprite_index);
    if (!renderer || slot < 0 || !g_type_a[slot].texture) return;
    sprite = &g_type_a[slot];
